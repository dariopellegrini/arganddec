...
function pong() {
	...
	$('#pong-modal').modal('show');
					...
					function beforeInit(){
						...
						ctx.fillText("Start PONG! with space and move with arrows.",50,50);
					} 
					...
					function draw() {
						  ...
						  if (y + dy - radius <= 0) {
							...
							if (x <= paddlexAI || x >= paddlexAI + paddlew) {
								...
								bootbox.alert("<h3> YOU WIN! How about another PONG! game?</h3>");
								beforeInit();
						  ...
						  //lower lane
							...
							if (x > paddlex && x < paddlex + paddlew) {
							  ...
							  clearInterval(intervalId);
								bootbox.alert("<h3> YOU LOSE! How about try another time?</h3>");
								beforeInit();
//							  init();
							}
						  }
